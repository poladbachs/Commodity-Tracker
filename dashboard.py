...
def main():
    ...
    commodities = {
        ...
        "Steel": "SLX"        # Example: VanEck Steel ETF (placeholder)
    }

    # We'll map each commodity to more specific keywords for the news API
    news_search_terms = {
    ...
    # For each selected commodity, we fetch data, do analysis, and show news
    for commodity in selected:
        ticker = commodities[commodity]

        # Container for the entire commodity section
        with st.container():
            st.markdown(f"<h2 style='margin-top:20px;'>{commodity}</h2>", unsafe_allow_html=True)

            # Top row: left = analysis, right = news
            ...
            with col_left:
                ...
                forecast = forecast_prices(price_data, 7)
            ...
            with col_right:
                ...
                if headlines:
                    for headline in headlines:
                        st.markdown(f"<div class='news-item'>{headline}</div>", unsafe_allow_html=True)
                else:
